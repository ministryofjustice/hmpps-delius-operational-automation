#!/bin/bash
#
#  Create DELIUS_AUDIT_SCHEMA.COMPRESS_AUDITED_INTERACTION job to periodically compress old AUDITED_INTERACTION partitions.
...
BEGIN
    ...
    WHERE  owner = 'DELIUS_AUDIT_SCHEMA'
    AND    job_name = 'COMPRESS_AUDITED_INTERACTION';
    
    IF l_existing_job > 0
    THEN
       DBMS_SCHEDULER.drop_job (
         job_name => 'DELIUS_AUDIT_SCHEMA.COMPRESS_AUDITED_INTERACTION'
       );
       DBMS_SCHEDULER.drop_schedule (
         schedule_name => 'DELIUS_AUDIT_SCHEMA.COMPRESSION_SCHEDULE'
       );       
       DBMS_SCHEDULER.drop_program (
         program_name => 'DELIUS_AUDIT_SCHEMA.COMPRESS_AUDIT_PROG'
       ); 
    END IF;
END;
/

-- Enable required privileges for the DELIUS_AUDIT_SCHEMA
-- to contain a job for compressing the AUDITED_INTERACTION TABLE

GRANT SELECT  ON dba_tab_partitions                    TO delius_audit_schema;
GRANT ALTER   ON delius_app_schema.audited_interaction TO delius_audit_schema;
GRANT ANALYZE ANY                                      TO delius_audit_schema;
...
BEGIN
    ...
    THEN
        DBMS_SCHEDULER.create_program(
            ...
            enabled => FALSE);
        DBMS_SCHEDULER.ENABLE(name=>'DELIUS_AUDIT_SCHEMA.COMPRESS_AUDIT_PROG');   
   END IF;
END;
/
