    ...
    * custom_properties: Properties which are not supported by EMCLI Metrics Input Files
    ...
    for k, v in d.items():
        ...
        if isinstance(v, dict):
            items.extend(flatten_emcli_metrics(
                v, config_type, new_keys, separator=separator))
        else:
            if config_type == 'metric' and new_keys[-1] == 'input_file':
                item = {'target_type': None,
                        'target_name': None, 'input_file': v}
                if len(new_keys) >= 3:
                    item['target_name'] = new_keys[-2]
                    item['target_type'] = new_keys[-3]
                items.append(item)
            elif config_type == 'schedule' and new_keys[-3] == 'schedule':
                item = {'target_type': None, 'target_name': None, 'collection_name': None,
                        'collection_attribute': None, 'collection_attribute_value': v}
                if len(new_keys) >= 5:
                ...
                item = {'target_type': None, 'target_name': None,
                        'property_name': None, 'property_value': v}
                if len(new_keys) >= 4:
